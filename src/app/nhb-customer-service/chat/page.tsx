...
interface Message {
  ...
  role: 'user' | 'bot';
  content: string;
  type?: 'reply' | 'auth'; // 消息类型：reply=普通回复，auth=实名认证
  verifyUrl?: string; // 实名认证地址
  hasFeedback?: boolean;
  helpful?: boolean;
  timestamp?: number;
}
...
export default function CustomerServicePage() {
    ...
    try {
      ...
      if (result.code === 200) {
        ...
        if (result.data.isNewUser && result.data.userId) {
          localStorage.setItem(USER_ID_KEY, result.data.userId.toString());
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            type: result.data.type || 'reply',
            verifyUrl: result.data.verifyUrl,
            hasFeedback: result.data.type !== 'auth', // 实名认证消息不需要反馈按钮
            timestamp: Date.now(),
          },
        ]);
      } else {
  ...
  const clearHistory = async () => {
    if (!userId) return;
    try {
      await fetch(`/nhb-customer-service-api/history?userId=${userId}&channel=${channelRef.current}`, { method: 'DELETE' });
      setMessages([]);
    } catch (error) {
