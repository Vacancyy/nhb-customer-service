          ...
          {!loadingHistory && messages.map((msg) => (
            <div key={msg.id} className={`flex gap-2 mb-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={msg.role === 'user' ? 'max-w-[70%]' : 'max-w-[80%]'}>
                ...
                {msg.role === 'bot' && (
                  <div className="bg-white rounded-xl shadow-sm text-sm max-w-xs">
                    <div className="px-3 py-2 text-gray-800">
                      {msg.status ? (
                        <div className="flex items-center gap-2">
                          <div className="flex gap-1">
                            <span className="w-1.5 h-1.5 bg-teal-500 rounded-full animate-pulse" style={{ animationDelay: '0ms' }} />
                            <span className="w-1.5 h-1.5 bg-teal-500 rounded-full animate-pulse" style={{ animationDelay: '150ms' }} />
                            <span className="w-1.5 h-1.5 bg-teal-500 rounded-full animate-pulse" style={{ animationDelay: '300ms' }} />
                          </div>
