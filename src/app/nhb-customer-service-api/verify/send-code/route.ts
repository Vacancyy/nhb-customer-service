...
import { generateCode, storeCodeA, storeCodeB, sendSMS_A, sendSMS_B, verifyThreeElements } from '@/lib/verification-code';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingAuth) {
      // 已实名认证 → 方法A（普通短信）
      await storeCodeA(phone, code);
      await sendSMS_A(phone, code);
      ...
      const verifyResult = await verifyThreeElements(name, idCard, phone);
      ...
      await storeCodeB(phone, code);
      await sendSMS_B(phone, code);

      logInfo('[send-code] 方法B发送验证码', { phone, name, alreadyVerified: false });
