...
import {verifyCodeMiaoXin, verifyThreeElementsResult, getThreeElementsData} from '@/lib/verification-code';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (existingAuth) {
      // 已实名认证 → 方法A验证验证码
      verified = await verifyCodeMiaoXin(phone, code);
      if (!verified) {
      ...
      // 未实名认证 → 方法B验证验证码
      // 从 Redis 获取三要素认证数据（flowId）
      const flowId = await getThreeElementsData(phone);
      if (!flowId) {
        return NextResponse.json(AjaxResult.error('认证会话已过期，请重新发送验证码', 400));
      }

      verified = await verifyThreeElementsResult(flowId as string, code);
      if (!verified) {
        return NextResponse.json(AjaxResult.error('验证码不正确或已过期', 400));
      }
