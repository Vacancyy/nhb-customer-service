...
async function runAgentLoopPending(
  ...
  channel: string,
  trace: LangfuseTraceClient | null
): Promise<{ reply: string; recordId: number | null; type?: string; verifyUrl?: string }> {
  // 获取用户实名认证信息
  ...
  const recordId = await saveConversationToHistory(userId, channel, userMessage, fullReply, 'pending');

  return { reply: fullReply, recordId };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 执行 Agent Loop，生成完整回答并保存为 pending
    const result = await runAgentLoopPending(message, session.userId, session.channel, trace);
