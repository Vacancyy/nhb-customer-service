// 更新Redis中的系统提示词，使用包含意图澄清的最新版本
import dotenv from 'dotenv';
import path from 'path';

dotenv.config({ path: path.join(__dirname, '../', '.env.local') });

import { setSystemPrompt } from '../src/lib/prompts';
...
3. 如果用户问的是具体数值或期限（如犹豫期、保费、电话），必须明确给出答案`;
  ...
  console.log('更新Redis中的系统提示词...\n');
  console.log('提示词长度:', newPrompt.length);

  await setSystemPrompt(newPrompt);

  console.log('✅ 系统提示词已更新到Redis');
  console.log('\n现在可以测试意图澄清功能了！');
  console.log('访问: http://localhost:3001/nhb-customer-service/web/app/chat');
  console.log('测试问题:');
  console.log('  - "多少钱？"');
  console.log('  - "什么时候？"');
  console.log('  - "怎么弄？"');
